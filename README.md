...
not the sole auditor who promotes it (R1–R28).
...
examples/                       worked registry that passes R1–R28;
...
attribution of that spend to an entry is a human judgement; and the claim that
the tool *caused* a difference needs a comparison arm. R22 asks for all three
and caps the entry below `[K]` when `baseline.kind` is `none` — R2 pointed at
the claim teams most often make without a control. Also ships
`tools/token_budget.py` in all four repos: the context cost of the skill itself
(the always-on description, the body, the on-demand files, and what one run
actually loads) now carries preregistered ceilings and a CI gate, because
between v2.3 and v2.5 the body grew 13% and a registry-writing run 31% with
nothing watching. Registry schema 1.9.
