...
### Ground rules (the same R1–R22 the tool enforces)
...
# 1. registries must pass R1–R22 (English or Turkish messages):
...
- **validate-registries** — runs the R1–R22 validator on every
...
- `tools/` — the R1–R22 validator and git hook (MIT).
- `examples/` — worked registries that MUST stay R1–R22-clean.
...
### Temel kurallar (aracın uyguladığı R1–R22'nin aynısı)
...
- **validate-registries** — R1–R22 validator'ını her `*mizan-registry*.yaml`
...
- `tools/` — R1–R22 doğrulayıcı ve git hook (MIT).
- `examples/` — R1–R22'den temiz kalması ZORUNLU çalışan registry'ler.
