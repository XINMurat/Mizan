...
### Ground rules (the same R1–R21 the tool enforces)
...
# 1. registries must pass R1–R21 (English or Turkish messages):
...
- **validate-registries** — runs the R1–R21 validator on every
...
- `tools/` — the R1–R21 validator and git hook (MIT).
- `examples/` — worked registries that MUST stay R1–R21-clean.
...
### Temel kurallar (aracın uyguladığı R1–R21'nin aynısı)
...
- **validate-registries** — R1–R21 validator'ını her `*mizan-registry*.yaml`
...
- `tools/` — R1–R21 doğrulayıcı ve git hook (MIT).
- `examples/` — R1–R21'den temiz kalması ZORUNLU çalışan registry'ler.
