...
### Ground rules (the same R1–R28 the tool enforces)
...
# 1. registries must pass R1–R28 (English or Turkish messages):
...
- **validate-registries** — runs the R1–R28 validator on every
...
- `tools/` — the R1–R28 validator and git hook (MIT).
- `examples/` — worked registries that MUST stay R1–R28-clean.
...
### Temel kurallar (aracın uyguladığı R1–R28'nin aynısı)
...
- **validate-registries** — R1–R28 validator'ını her `*mizan-registry*.yaml`
...
- `tools/` — R1–R28 doğrulayıcı ve git hook (MIT).
- `examples/` — R1–R28'den temiz kalması ZORUNLU çalışan registry'ler.
